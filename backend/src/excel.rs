...
// Excel and compatible suites (e.g. WPS) treat cells whose text starts with
// '=', '+', '-', '@' (and tab/CR) as formulas. Prefixing such user-controlled
// text with a single quote forces it to be displayed as a literal value, so
// a crafted source/purpose/item cannot smuggle a formula into the workbook.
// Full-width variants (＝＋－＠) are included because localized spreadsheet
// software may normalize them to the half-width formula markers.
...
pub fn build_excel(transactions: &[Transaction], report: &Report) -> anyhow::Result<Vec<u8>> {
    ...
    for item in report.item_ranking.iter().take(10) {
        summary.write_string(row, 0, sanitize_excel_text(&item.item))?;
    ...
    for (i, tx) in transactions.iter().enumerate() {
        ...
        data.write_string(r, 1, sanitize_excel_text(&tx.kind))?;
        data.write_number(r, 2, tx.amount_cents as f64 / 100.0)?;
        data.write_string(r, 3, sanitize_excel_text(tx.source.as_deref().unwrap_or("")))?;
        data.write_string(r, 4, sanitize_excel_text(tx.purpose.as_deref().unwrap_or("")))?;
        data.write_string(r, 5, sanitize_excel_text(tx.item.as_deref().unwrap_or("")))?;
        data.write_string(r, 6, sanitize_excel_text(&tx.operator))?;
        data.write_string(r, 7, sanitize_excel_text(&tx.occurred_at))?;
    }
