...
pub struct Config {
    ...
    // Optional bearer token that gates the /metrics endpoint. When unset,
    // /metrics is only reachable from loopback so internal metrics stay
    // private on a LAN-deployed instance.
