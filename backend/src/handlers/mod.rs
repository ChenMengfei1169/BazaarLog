...
pub fn build_router(state: AppState) -> axum::Router<()> {
    axum::Router::new()
        ...
        // Per-IP rate limiting; the strict budget guards the password-checking
        // endpoints. State is bound here because the middleware extracts
        // AppState before the router's final .with_state() runs.
        .layer(axum::middleware::from_fn_with_state(
            state.clone(),
            security::apply_rate_limit,
        ))
        // Explicit cross-origin mutation rejection (Origin vs Host check).
        .layer(axum::middleware::from_fn(security::reject_cross_origin_mutations))
        ...
        // the 429 replies produced by the rate limiter above.
        .layer(axum::middleware::from_fn(security::add_security_headers))
        // Bound how long any single request may run so a stalled client or a
...
// Detects UNIQUE constraint violations (SQLite code 2067, PostgreSQL SQLSTATE
// 23505) so handlers can answer with 409 instead of a generic 500.
