...
GET /metrics requires a match on the Authorization: Bearer <token> header when
BAZAARLOG_METRICS_TOKEN is set; otherwise only loopback clients may read it.
...
| Method | Path                              | Auth  | Description                                                                                           |
|--------|-----------------------------------|-------|-------------------------------------------------------------------------------------------------------|
| GET    | `/api/semesters/:id/transactions` | class | List transactions. Query: `kind`, `from`, `to`, `search`, `page`, `page_size`.                     |
